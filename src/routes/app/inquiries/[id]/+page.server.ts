...
async function authorizeResolution(inquiryId: string, userId: string) {
  const inquiryCheck = await maybeOne<{
    authority_id: string | null;
    status: string;
  }>(sql`
  ...
  if (
    inquiryCheck.status === "resolved" ||
    inquiryCheck.status === "dismissed"
  ) {
...
export const load: PageServerLoad = async ({ params, locals, parent }) => {
  ...
  const canResolve =
    isActive &&
    (locals.user?.id === inquiry.authority_id || canResolveAnyInquiry(role));
...
export const actions: Actions = {
  ...
  resolve: async ({ request, params, locals }) => {
    ...
    const resolutionAction = formData.get("resolutionAction")?.toString() as
      ...
      | "no_change"
      | "deferred";
    const resolutionSummary = formData
