...
export const actions: Actions = {
   ...
   */
  createMetric: async ({ request, locals }) => {
   ...
   */
  updateMetric: async ({ request, locals }) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
   ...
   */
  updateWeights: async ({ request, locals }) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
   ...
   */
  submitMetric: async ({ request, locals }) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
   ...
   */
  recordTier: async ({ request, locals }) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
   ...
   */
  setCadence: async ({ request, locals }) => {
    if (!locals.user) return fail(401);
