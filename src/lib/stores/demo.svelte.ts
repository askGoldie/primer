...
function createDemoStore() {
  ...
  const compositeTier = $derived(
    compositeScore !== null ? getTierFromScore(compositeScore) : null,
  );
  ...
  return {
    ...
    updateThreshold(id: string, tier: TierLevel, description: string) {
      metrics = metrics.map((m) =>
        m.id === id
          ? { ...m, thresholds: { ...m.thresholds, [tier]: description } }
          : m,
      );
    },
    ...
    setWeight(id: string, weight: number) {
      ...
      metrics = metrics.map((m) =>
        m.id === id ? { ...m, weight: clampedWeight } : m,
      );
    },
