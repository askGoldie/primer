...
export function calculateMetricDetails(
  ...
  return metrics.map((m) => {
    ...
    return {
      ...
      metricName: m.name,
      measurementType:
        m.measurementType as MetricScoreDetail["measurementType"],
 ...
 */
export function getTierBackgroundColor(
  tier: TierLevel,
  isDark = false,
): string {
  const colorMap: Record<TierLevel, { light: string; dark: string }> = {
    alarm: {
      ...
      dark: "rgba(240, 135, 126, 0.15)",
    },
    concern: {
      light: "rgba(122, 74, 0, 0.1)",
      dark: "rgba(245, 168, 66, 0.15)",
    },
    content: {
      light: "rgba(107, 96, 88, 0.1)",
      dark: "rgba(168, 158, 148, 0.15)",
    },
    effective: {
      light: "rgba(44, 110, 166, 0.1)",
      dark: "rgba(106, 170, 216, 0.15)",
    },
    optimized: {
      light: "rgba(42, 122, 69, 0.1)",
      dark: "rgba(90, 173, 120, 0.15)",
    },
  };
