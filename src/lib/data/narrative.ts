...
import type { Industry, Locale, VisitorRole, TierLevel } from '$lib/types/index.js';
 ...
 */
export function getIndustryCharacters(industry: Industry, locale: Locale): IndustryCharacters {
	const coreMetricsByIndustry: Record<Industry, string[]> = {
		construction: ['schedule adherence', 'budget variance', 'win rate', 'incident frequency rate'],
		healthcare: [
			'patient throughput',
		...
		professional_services: ['project margin', 'utilization rate', 'win rate', 'voluntary turnover'],
		manufacturing: [
			'on-time delivery',
			'cost of quality',
			'win rate',
		...
		healthcare: [
			...
			},
			{
				...
				threshold: {
					...
					optimized: 'Top quartile cost efficiency with quality metrics maintained'
				}
			},
			{
 ...
 */
export function getPartContent(part: NarrativePart, industry: Industry, locale: Locale): string[] {
	...
	const metric_concern_1 = concernMetrics[0]?.name.toLowerCase() || 'first concern metric';
	const metric_concern_2 = concernMetrics[1]?.name.toLowerCase() || 'second concern metric';
