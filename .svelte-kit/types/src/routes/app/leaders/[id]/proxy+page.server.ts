...
export const actions = {
	 ...
	 */
	approve: async ({ request, params, locals }: import('./$types').RequestEvent) => {
		if (!locals.user) {
	 ...
	 */
	assignMetric: async ({ request, params, locals }: import('./$types').RequestEvent) => {
	 ...
	 */
	editMetric: async ({ request, params, locals }: import('./$types').RequestEvent) => {
		if (!locals.user) return fail(403, { error: 'error.generic' });
	 ...
	 */
	captureSnapshot: async ({ request, params, locals }: import('./$types').RequestEvent) => {
		if (!locals.user) return fail(403, { error: 'error.generic' });
	 ...
	 */
	adjustSnapshot: async ({ request, params, locals }: import('./$types').RequestEvent) => {
		if (!locals.user) return fail(403, { error: 'error.generic' });
	 ...
	 */
	startNewCycle: async ({ params, locals }: import('./$types').RequestEvent) => {
		if (!locals.user) return fail(403, { error: 'error.generic' });
