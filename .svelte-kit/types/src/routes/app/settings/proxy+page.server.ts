...
} from "$lib/hierarchy/templates.js";
...
export const load = async ({ parent }: Parameters<PageServerLoad>[0]) => {
  ...
  const canViewAudit =
    role === "owner" || role === "system_admin" || role === "hr_admin";
  ...
  if (showVisibility) {
    ...
    if (userNode) {
      ...
      peerVisibilitySetting =
        (currentNodeRecord?.peer_visibility as
          | "score_only"
          | "metrics"
          | "full") ?? "score_only";
    }
    ...
    grants = grantsRaw.map((g) => {
      ...
      const scopeNode = g.scope_node_id
        ? allNodes.find((n) => n.id === g.scope_node_id)
        : null;
      return {
        id: g.id,
  ...
  if (canViewAudit) {
    ...
    if (isSystemAdmin) {
      ...
      for (const dept of deptNodes) {
        ...
        const deptNodeSet = [
          auditNodes.find((n) => n.id === dept.id),
          ...descendants,
        ].filter(Boolean) as AuditNodeRow[];
        ...
        departmentOverview.push({
          ...
          pendingReviews: deptNodeSet.reduce(
...
export const actions = {
  ...
  updateCycleCadence: async ({ request, locals }: import('./$types').RequestEvent) => {
    ...
    const formData = await request.formData();
    const cycleCadence = formData.get("cycleCadence")?.toString() as
      | "monthly"
      | "quarterly";
  ...
  updateRole: async ({ request, locals }: import('./$types').RequestEvent) => {
    ...
    const callerMembership = await maybeOne<{
      role: string;
    ...
    if (
      !callerMembership ||
      !canAssignRoles(callerMembership.role as OrgRole)
    ) {
      return fail(403, { error: "error.generic" });
    }
  ...
  updatePeerVisibility: async ({ request, locals }: import('./$types').RequestEvent) => {
    ...
    if (
      !peerVisibility ||
      !["score_only", "metrics", "full"].includes(peerVisibility)
    ) {
      return fail(400, { error: "validation.field_required" });
    }

    const membershipData = await maybeOne<{
      organization_id: string;
      role: string;
    }>(sql`
    ...
    if (
      !membershipData ||
      !canManageVisibility(membershipData.role as OrgRole)
    ) {
      return fail(403, { error: "error.generic" });
    }
  ...
  createHierarchyNode: async ({ request, locals }: import('./$types').RequestEvent) => {
    ...
    const nodeType = fd.get("nodeType")?.toString() as
      | HierarchyNodeType
      | undefined;
    ...
    if (
      !nodeType ||
      !["executive_leader", "department", "team", "individual"].includes(
        nodeType,
      )
    ) {
      return fail(400, { error: "validation.field_required" });
  ...
  populateHierarchyTemplate: async ({ request, locals }: import('./$types').RequestEvent) => {
    ...
    const fd = await request.formData();
    const executiveCount = parseInt(
      fd.get("executiveCount")?.toString() ?? "3",
      10,
    );
  ...
  restoreHierarchyNode: async ({ request, locals }: import('./$types').RequestEvent) => {
    ...
    if (childIds.length > 0) {
      ...
      for (const c of childRows) {
        if (
          !validateContainment(c.node_type as HierarchyNodeType, node.node_type)
        ) {
          return fail(400, { error: "hierarchy.invalid_containment" });
        }
      }
  ...
  bulkCreateHierarchyNodes: async ({ request, locals }: import('./$types').RequestEvent) => {
    ...
    const existingNodes = await many<{
      id: string;
      name: string;
      node_type: string;
    }>(sql`
    ...
    const nameMap = new Map<
      string,
      { id: string; nodeType: HierarchyNodeType }
    >();
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (row.parentName) {
        ...
        if (
          !validateContainment(
            row.nodeType as HierarchyNodeType,
            parent.nodeType,
          )
        ) {
          failureCount++;
