...
export const load = async ({ parent, url }: Parameters<PageServerLoad>[0]) => {
  const { organization, userNode, membership, hasDirectReports } =
    await parent();

  const role = membership.role;
  const isAdminRole =
    role === "system_admin" || role === "owner" || role === "hr_admin";
  ...
  if (scopeParam === "org" && isAdminRole) {
    ...
    if (isAdminRole) {
      ...
      if (subtree.includes(targetNodeId)) {
        ...
        if (hasGrant) {
          scope = "subtree";
          const childIds = await getSubtreeNodeIds(
            targetNodeId,
            organization.id,
          );
          scopeNodeIds = [targetNodeId, ...childIds];
  ...
  if ((isOrgWide || scope === "subtree") && snapshots.length > 0) {
    ...
    scoreTrend = Array.from(byCycle.entries()).map(
      ([label, { scores, date }]) => {
  ...
  const metricHistory = Array.from(metricGroups.entries()).map(
    ([metricId, entries]) => ({
      metricId,
...
export const actions = {
  exportSubtree: async ({ request, locals }: import('./$types').RequestEvent) => {
