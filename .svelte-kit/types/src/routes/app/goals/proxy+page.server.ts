...
export const load = async ({ parent }: Parameters<PageServerLoad>[0]) => {
  const { organization, userNode, hasDirectReports, membership } =
    await parent();
  ...
  if (hasDirectReports && goals.length > 0) {
    ...
    const cascadedGoals = await many<{
      source_goal_id: string | null;
      status: string;
    }>(sql`
    ...
    for (const cg of cascadedGoals) {
      ...
      if (!cascadeStatus[cg.source_goal_id]) {
        cascadeStatus[cg.source_goal_id] = {
          total: 0,
          accepted: 0,
          deferred: 0,
          pending: 0,
...
export const actions = {
   ...
   */
  createMetric: async ({ request, locals }: import('./$types').RequestEvent) => {
    ...
    const maxSortOrder = Math.max(
      0,
      ...existingMetrics.map((m) => m.sort_order),
    );
   ...
   */
  updateMetric: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) return fail(403, { error: "error.generic" });

    const ctx = await loadActionContext(locals.user.id);
    if (!ctx || ctx.membership.role === "viewer")
      return fail(403, { error: "error.generic" });
    ...
    if (await isMetricLocked(metricId))
      return fail(403, { error: "metric.locked" });
   ...
   */
  deleteMetric: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
   ...
   */
  updateWeights: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
    ...
    const weights = JSON.parse(
      formData.get("weights")?.toString() || "{}",
    ) as Record<string, number>;

    const total = Object.values(weights).reduce((sum, w) => sum + w, 0);
    if (total !== 100)
      return fail(400, { error: "validation.weights_must_total" });
   ...
   */
  submitMetric: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
   ...
   */
  recordTier: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
   ...
   */
  createGoal: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
    ...
    const targetTier = formData.get("targetTier")?.toString() as
      | TierLevel
      | undefined;
   ...
   * audit entry.
   */
  updateGoal: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
    ...
    const targetTier = formData.get("targetTier")?.toString() as
      | TierLevel
      | undefined;
    const actualTier = formData.get("actualTier")?.toString() as
      | TierLevel
      | undefined;
    const dueDate = formData.get("dueDate")?.toString() || null;
    ...
    const validatedType = VALID_GOAL_TYPES.includes(goalType)
      ? goalType
      : current.goal_type;
    const validatedPriority = VALID_PRIORITIES.includes(priority)
      ? priority
      : current.priority;
    const validatedStatus = VALID_STATUSES.includes(status)
      ? status
    ...
    const validatedTargetTier =
      targetTier && VALID_TIERS.includes(targetTier) ? targetTier : null;
    const validatedActualTier =
      actualTier && VALID_TIERS.includes(actualTier) ? actualTier : null;
    ...
    const fieldComparisons: {
      ...
      { field: "goal_type", oldVal: current.goal_type, newVal: validatedType },
      {
        ...
        newVal: validatedPriority,
      },
      { field: "status", oldVal: current.status, newVal: validatedStatus },
      {
        field: "target_tier",
        oldVal: current.target_tier,
        newVal: validatedTargetTier,
      },
      {
        field: "actual_tier",
        oldVal: current.actual_tier,
        newVal: validatedActualTier,
      },
   ...
   * Delete a goal.
   */
  deleteGoal: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
   ...
   */
  cascade: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) return fail(403, { error: "error.generic" });
    ...
    const childNodeIds = await getDirectChildNodeIds(
      userNode.id,
      organization.id,
    );
   ...
   */
  record: async ({ request, locals }: import('./$types').RequestEvent) => {
   ...
   */
  setCadence: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) return fail(401);
