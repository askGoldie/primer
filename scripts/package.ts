...
const manifest = JSON.parse(
  readFileSync(resolvePath(projectRoot, "package.json"), "utf8"),
);
...
try {
  ...
  console.error(
    "[package]   ✗ npm run check failed — fix errors before packaging.",
  );
  const stderr =
...
if (dirty) {
  ...
  console.warn(
    "[package]   git archive ships HEAD only — these changes will NOT be in the zip.",
  );
}
...
if (!ok) {
  console.error(
    "[package] zip contents do not match the expected shape — investigate .gitattributes.",
  );
  process.exit(1);
}
